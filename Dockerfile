...
COPY --from=build /app/target/Eureka-Server_Prometheus_Grafana-0.0.1-SNAPSHOT.jar /app/eureka-server-prometheus-grafana.jar

# Exponemos el puerto 8761 para Eureka
EXPOSE 8762

# Definimos el comando para iniciar Eureka
ENTRYPOINT ["java", "-jar", "/app/eureka-server-prometheus-grafana.jar"]
